...
class Operation {
	...
	startClients() {
		...
		const replaceUntil = this.options.maxRequests || this.options.concurrency;
		
		for (let index = 0; index < replaceUntil; index++) {
			if (this.options.indexParam) {
				this.options.url = url.replaceAll(this.options.indexParam, index);

				if(this.options.body) {
					let strBody = JSON.stringify(originalBody);
					strBody = strBody.replaceAll(this.options.indexParam, index);
					this.options.body = JSON.parse(strBody);
				}
			}
			if (this.options.randomFields) {
				for (let randomFieldsIndex = 0; randomFieldsIndex < this.options.randomFields.length; randomFieldsIndex++) {
					if(this.options.body && this.options.body[this.options.randomFields[randomFieldsIndex]]) {
						this.options.body[this.options.randomFields[randomFieldsIndex]] = uuidv4();
					}
				}
			}
